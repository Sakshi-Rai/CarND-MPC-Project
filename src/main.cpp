    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
		  ...
		  //ptsx,ptsy are the coordinates we have to follow
		  //px,py is the current position of car
          ...
          double psi_delay =0 - ( v * delta * delay /mpc.Lf );
          double v_delay = v + a * delay;
          double cte_delay = cte + ( v * sin(epsi) * delay );
          double epsi_delay = epsi - ( v * delta * delay /mpc.Lf );
